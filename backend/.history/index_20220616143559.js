...
const routes = require("./dialogflow.route");
                    ...
                    if(err){
                        ...
                        registeredUsers.users.forEach((user, index) => {
                            if(user.email == req.query.email){
                                ...
                                registeredUsers.users[index].password = hashPassword
        
                                registeredUsers = JSON.stringify(registeredUsers)

                                fs.writeFile('registered_users.json', registeredUsers, (err) => {
                                    if (err) {
                                        resp.status(204).setHeader("Message", "Erro ao atualizar dados do usuário.").send();
                                            return
                                    }else{
